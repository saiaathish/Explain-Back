...
import { outstandingCards, roundSummary } from "../flashcards";
import {
  markGapCleared,
  unmarkGapCleared,
  useLearningData,
...
export default function ReviewPage() {
  ...
  const { cards: allCards, cleared, sessions, status } = useLearningData();
  ...
  /* The deck is seeded once. After that the queue is the learner's working
     order, and the shared store must not shuffle it mid-round. */
  useEffect(() => {
    if (seededRef.current || status !== "ready") return;
    seededRef.current = true;
    setQueue(outstandingCards(allCards, cleared));
  }, [allCards, cleared, status]);
  ...
  const act = useCallback(
    (card, direction) => {
      ...
      setSaveError("");
      markGapCleared(card.sessionId, card.propId);
      getClearedGaps()
        .clearGap({ sessionId: card.sessionId, propId: card.propId })
        .catch(() => {
          unmarkGapCleared(card.sessionId, card.propId);
          if (!mountedRef.current) return;
    ...
    <section className="review-view" aria-labelledby="review-title">
      ...
      {status === "error" && (
        <p className="history-message history-message--error" role="alert">
          Your gaps could not be loaded. You can still start a new session.
        </p>
      )}
          ...
          {remaining > 0 ? (
            <>
              ...
              <div className="review-actions">
                <button className="primary" onClick={() => gotIt(top)} type="button">
                  Got it now
                </button>
                <button
                  className="secondary"
            ...
            <div className="review-done" role="status">
              ...
              <h2>{practice ? "Practice complete" : "Nothing left to explain"}</h2>
              <p>
                {practice
              ...
              <div className="review-actions">
                ...
                <button
                  className="secondary"
                  onClick={
                    practice ? leavePractice : () => navigate("/dashboard")
                  }
                  type="button"
                >
                  {practice ? "Back to my gaps" : "Back to dashboard"}
                </button>
              </div>
            </div>
          )}
